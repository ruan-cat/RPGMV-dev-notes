...
本次会话将专注于为我编写一个处理文件整理的 typescript 程序。
...
提供一个 typescript 编写的函数，该函数接收两个参数：

- inputPath: string 类型，表示要整理的文件夹路径
- outputPath: string 类型，表示整理后的文件夹路径

该函数将读取 inputPath 下的所有文件，并按照特定的规则进行整理。整理后的文件应该按照特定的目录结构进行存放。

最后，该函数应该将整理后的文件存放到 outputPath 指定的文件夹路径下。
...
第一层是业务名文件夹。第二层是年份文件夹。第三层是月份文件夹。在月份文件夹内，有一个或多个名称格式形如 `业务名.xlsx` 或 `业务名.xlsx` 表格文件。
...
第一层是年份文件夹。第二层是月份文件夹。在月份文件夹内，有一个或多个名称格式形如 `业务名.xlsx` 或 `业务名.xlsx` 表格文件。
