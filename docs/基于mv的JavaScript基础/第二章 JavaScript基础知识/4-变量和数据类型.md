...
undefined * 1
```
有时候你是无法有效预估这一类情形出现的，因此我们需要拦截并处理类似的非法值。```NaN```、```undefined```、```null```都是```js```内常见的非法值。可以使用以下的语句处理，提高程序的健壮性。
``` js
!!p;
```
使用连续的两个感叹号实现变量类型的布尔值类型转换，并得出结果。常见的非法值否会被转换成```false```。值得注意的是，0也被该语句识别为非法值，如果在你的代码内数值0也是实际意义的值，可以写成如下的形式。
``` js
!!p || p === 0;
```
